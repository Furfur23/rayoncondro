...
@section('content')
    ...
    <div class="bg-gray-900 border border-gold-700/40 rounded-2xl p-4 mb-5 shadow">
        <p class="text-xs text-gray-400 mb-1">Siswa Aktif</p>
        <p class="text-3xl font-bold text-gold-400">{{ $total_siswa_aktif }}</p>
        <p class="text-xs text-gray-500 mt-1">orang terdaftar</p>
    </div>

    {{-- Jadwal Hari Ini --}}
    <div class="bg-gray-900 border border-gray-800 rounded-2xl p-4 mb-5 shadow">
        ...
        @if($jadwal_hari_ini)
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-xs bg-gold-500/20 text-gold-400 border border-gold-600/40 px-3 py-1 rounded-full">
                    Ada Latihan
                </span>
            </div>
        ...
        @else
            <p class="text-gray-500 text-sm text-center py-2">😴 Tidak ada latihan hari ini</p>
        @endif
    </div>
